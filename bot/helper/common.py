...
class TaskConfig:
    ...
    async def proceed_compress(self, dl_path, gid):
        ...
        result = await sevenz.zip(dl_path, up_path, pswd)

        # ✅ Cleanup original downloaded folder
    ...
    async def proceed_split(self, dl_path, gid):
        # Skip split if ZIP already has parts (7z auto-split)
        if any(
            f.endswith((".zip.001", ".zip.002", ".zip.003"))
            for f in os.listdir(os.path.dirname(dl_path))
        ):
            LOGGER.info("⚠️ Skipping manual split: archive already multi-part.")
            return
        ...
        if not self.files_to_proceed:
            LOGGER.info("✅ No files need manual splitting.")
            return
        ...
        for f_path, (f_size, file_) in self.files_to_proceed.items():
            ...
            # Only run raw split if not ZIP or 7z archive
            if not f_path.endswith((".zip", ".7z")):
                self.progress = False
                res = await split_file(f_path, split_size, self)
            else:
                LOGGER.warning(f"Skipping redundant split for archive: {f_path}")
                res = True
