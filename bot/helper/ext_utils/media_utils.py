...
class FFMpeg:
    ...
    async def split(self, f_path, file_, parts, split_size):
        ...
        Smart splitter:
          ...
          - Uses byte-based splitting for ZIPs or archives
        """
        ...
        if extension in [".zip", ".rar", ".7z"]:
            LOGGER.info(f"📦 Using byte-based split for archive: {f_path}")
            part_size = split_size - 3_000_000
            part_num = 1
            try:
                async with aio_open(f_path, "rb") as src:
                    while True:
                        chunk = await src.read(part_size)
                        if not chunk:
                            break
                        out_path = f"{f_path}.part{part_num:03}{extension}"
                        async with aio_open(out_path, "wb") as part:
                            await part.write(chunk)
                        LOGGER.info(f"✅ Created: {out_path}")
                        self._last_processed_bytes += len(chunk)
                        part_num += 1
                with suppress(Exception):
                    await remove(f_path)
                return True
            except Exception as e:
                LOGGER.error(f"❌ Archive split failed: {e}")
                return False
        ...
        while i <= parts or start_time < duration - 4:
            ...
            if code == -9:
                ...
                if multi_streams:
                    LOGGER.warning(
                        f"{stderr}. Retrying without -map 0. Path: {f_path}"
                    )
                    multi_streams = False
                    continue
                else:
                    LOGGER.warning(
                        f"{stderr}. Unable to split this video. Path: {f_path}"
                    )
                return False

            out_size = await aiopath.getsize(out_path)
            if out_size > self._listener.max_split_size:
                split_size -= (out_size - self._listener.max_split_size) + 5_000_000
                LOGGER.warning(
                    f"Part size too large ({out_size}). Retrying with lower split size. Path: {f_path}"
                )
                await remove(out_path)
                continue
            ...
            if lpd == 0:
                LOGGER.error(f"❌ Corrupted part detected. Path: {f_path}")
                break
            elif duration == lpd:
                LOGGER.warning(
                    f"⚠ Only one part created, missing streams (common for MKV). Path: {f_path}"
                )
                break
            elif lpd <= 3:
                await remove(out_path)
