...
async def clean_all():
    LOGGER.info("Cleaning Download Directory without TorrentManager")
    # Directly clean the DOWNLOAD_DIR without touching TorrentManager
...
async def split_file(f_path, split_size, listener):
    ...
    # Skip archives (they’re handled elsewhere)
    if f_path.endswith((".zip", ".7z")):
        LOGGER.warning(f"Skipping raw split for archive: {f_path}")
        return True
    ...
    listener.subproc = await create_subprocess_exec(
        ...
        f"--bytes={split_size}",
        f_path,
        os.path.join(folder_path, f"{stem}.part"),
        stderr=PIPE,
    )
    ...
    # ✅ Rename files from video.part001 → video.001.mp4
    for file in sorted(os.listdir(folder_path)):
        if file.startswith(f"{stem}.part"):
            old_path = os.path.join(folder_path, file)
            suffix = file.replace(f"{stem}.part", "")
            new_path = os.path.join(folder_path, f"{stem}.{suffix}{ext}")
            os.rename(old_path, new_path)

    LOGGER.info(f"✅ Split completed for: {f_path}")
    return True
